...
impl RewardsContract {
    ...
    /// - HostOnly: Only the original funder (authority) can add more funds
    ...
    /// controlled solely by the first funder. Contributors have no withdrawal mechanism
    ...
    pub fn fund_quest(
        ...
        // If quest already has an authority, check funding model
        if let Some(existing_authority) = env.storage().persistent().get::<_, Address>(&auth_key) {
            // Get the stored funding model
            ...
            // Enforce funding model
            match stored_model {
                FundingModel::HostOnly => {
                    if existing_authority != funder {
                        return Err(Error::Unauthorized);
                    }
                }
                FundingModel::Anyone => {
                    // Anyone can fund, no restriction
                }
            }
        } else {
            // First time funding: verify that the funder is the quest owner
            ...
            // First time funding: set authority and funding model
